...
function queryFilter(filter, search){
    ...
    let sqlquery= "Select distinct * FROM cs_2300_db.title;";
    if(filter == 'title'){
        sqlquery = `Select distinct * FROM cs_2300_db.title where T_name like '%${search}%';`;
    }else if(filter == 'genre'){
        sqlquery = `select distinct T.*  from cs_2300_db.title as T, cs_2300_db.genre_of as G where T.T_id = G.T_id and G_name like'%${search}%';`
    }else if(filter == 'name'){
        sqlquery = `select distinct T.*  from cs_2300_db.title as T where T.T_id in(select A.T_id from cs_2300_db.acted_by as A, cs_2300_db.person as P  where P.name like '%${search}%' and A.A_id = P.id)or
        ...
        sqlquery = `select distinct T.*  from cs_2300_db.title as T where T.T_id in(select O.T_id from cs_2300_db.streaming_on as O, cs_2300_db.streaming_service as S where S.S_name like '%${search}%' and S.S_id = O.S_id);`
    }else if(filter == 'id'){
